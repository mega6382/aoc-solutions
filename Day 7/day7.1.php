...
function processAllSteps(string $completedStep, array $processed, string $result, array $availableSteps = [])
{
	foreach($processed as $key => $value)
	{
		if($completedStep == $key)
		{
			$availableSteps = array_merge($availableSteps, $value);
			$resultArray = str_split($result);
			unset($processed[$key]);
			sort($availableSteps);
			foreach($availableSteps as $stepKey => $availableStep)
			{
				if(in_array($availableStep, $resultArray))
				{
					continue;
				}
				if(in_array($availableStep, flatten($processed)))
				{
					continue;
				}
				$result .= $availableStep;
				$completedStep = $availableStep;
				unset($availableSteps[$stepKey]);
				break;
			}
			$result = processAllSteps($completedStep, $processed, $result, $availableSteps);
		}
	}
	return $result;
}

array_walk($input, function($item) use (&$processed) {
	preg_match('#Step ([A-Z]) must be finished before step ([A-Z]) can begin.#', $item, $matches);
	$processed[$matches[1]][] = $matches[2];
});


$firstStepNominees = [];

$flattenedProcessed = flatten($processed);
array_walk($processed, function($item, $key) use (&$firstStepNominees, $flattenedProcessed){
	if(!in_array($key, $flattenedProcessed))
	{
		$firstStepNominees[] = $key;
	}
});
sort($firstStepNominees);

$firstStep = $firstStepNominees[0];
unset($firstStepNominees[0]);

$result = processAllSteps($firstStep, $processed, $firstStep, $firstStepNominees);
